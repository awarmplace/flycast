...
cmake -B build-release -G Ninja -DCMAKE_BUILD_TYPE=Release -DCMAKE_C_COMPILER=cl -DCMAKE_CXX_COMPILER=cl -DUSE_DX9=OFF -DUSE_DX11=OFF
if errorlevel 1 (echo CMAKE CONFIGURE FAILED & exit /b 1)
start /LOW /WAIT /B cmake --build build-release --target flycast -- -j %JOBS%
